...
function walkNode(node, filePath, config, nodes, edges, classCtx) {
  ...
  // --- IMPL block (Rust `impl Type { ... }`) — functions inside belong to the type ---
  if (nt.impl && nt.impl.includes(nodeType)) {
    const implTarget = node.childForFieldName('type');
    const implName = implTarget ? implTarget.text : null;
    if (implName) {
      for (let i = 0; i < node.childCount; i++) {
        walkNode(node.child(i), filePath, config, nodes, edges, implName);
      }
      return;
    }
  }
