...
export function findGodNodes(graph, options = {}) {
  ...
  if (options.minDegree !== undefined) {
    ...
    // P99 of degree distribution, minimum 8. A floor of 50 made this a
    // no-op on normal-sized repos where P99 degree is typically 5-25.
    const sorted = [...degrees].sort((a, b) => a - b);
    const p99idx = Math.floor(sorted.length * 0.99);
    threshold = Math.max(8, sorted[p99idx] ?? 0);
  }
