...
export class BrainManager {
  ...
  async saveIndex(index) {
    // Atomic write: a crash mid-write must not leave index.json truncated,
    // because loadIndex silently falls back to an empty index on parse failure.
  ...
  async createEntry({ type, prefix, dirName, title, frontmatter, body }) {
    ...
    await this.withLock(async () => {
      ...
      // Store rule for lessons (fast brain_get_lessons / firewall lookup)
      if (frontmatter.rule) {
        indexEntry.rule = typeof frontmatter.rule === 'string'
          ? frontmatter.rule.replace(/^"|"$/g, '').replace(/\\"/g, '"')
          : frontmatter.rule;
      }
    ...
    if (!result) {
      throw new Error('createEntry: operation did not produce a result (lock contention?)');
    }
  ...
  async syncRecentToClaudeMd() {
    ...
    try {
      ...
      if (recent.length > 0) {
        ...
        recentSection = '_No entries yet._';
      }
  ...
  async updateEntry(id, updates) {
    ...
    await this.withLock(async () => {
      ...
      if (updates.content) {
        // Match the closing --- only at line start so YAML values containing
        // "---" cannot sever the frontmatter.
  ...
  async appendChangelog(message) {
    // Serialize changelog writes through an in-process queue: concurrent tool
    // calls would otherwise read-modify-write the same content and lose lines.
    this._changelogQueue = (this._changelogQueue || Promise.resolve()).then(
      () => this._appendChangelogUnsafe(message),
      () => this._appendChangelogUnsafe(message)
    );
    return this._changelogQueue;
  }

  async _appendChangelogUnsafe(message) {
  ...
  async rebuildIndex() {
    const dirs = [
      ...
      { dir: 'research', type: 'research', prefix: 'RES' }
    ];
    const entries = [];
    const counters = { DEC: 0, BUG: 0, IMPL: 0, PAT: 0, PLAN: 0, LES: 0, RES: 0 };
    ...
    // Rebuilding replaced index.json wholesale, which dropped the persisted
    // cognitive-firewall rules — re-extract them so preflight keeps working.
    try {
      await this.rebuildRuleIndex();
    } catch (err) {
      process.stderr.write(`[brain] Warning: rule index rebuild failed: ${err.message}\n`);
    }
  ...
  async initBrain({ projectName, overview, paths }) {
    // Create directories
    const dirs = ['decisions', 'implementations', 'bugs', 'patterns', 'plans', 'lessons', 'research', 'history'];
