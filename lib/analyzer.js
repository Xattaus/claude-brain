...
function generateDescription(analysis) {
  ...
  if (analysis.types.length > 0) {
    parts.push(`${analysis.types.join(' / ')}  project`);
  } else {
    parts.push('Project');
  }
...
export function generateOverview(analysis) {
  ...
  if (analysis.pathAnalyses && analysis.pathAnalyses.length > 0) {
    md += `## Project Locations\n\n`;
    md += `| Location | Label | Type |\n`;
    md += `|----------|--------|--------|\n`;
  ...
  if (analysis.testDirs.length > 0) {
    md += `## Tests\nTest directories: ${analysis.testDirs.join(', ')}\n\n`;
  }

  md += `---\n*Auto-generated ${new Date().toISOString().substring(0, 10)}*\n`;
