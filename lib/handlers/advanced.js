...
export const advancedHandlers = {
  ...
  brain_visualize: async (ctx, args) => {
    ...
    const projectPath = ctx.manager?.projectPath || process.env.BRAIN_PROJECT_PATH;
    if (!projectPath) {
      return [{ type: 'text', text: 'Error launching visualizer: project path is not available.' }];
    }

    try {
      // Fork the visualizer (it opens the browser itself)
      const child = fork(vizScript, [projectPath], { silent: true });
      const result = await waitForVisualizerUrl(child);
      if (!result.url) {
        return [{ type: 'text', text: `Error launching visualizer: ${result.error}` }];
      }
...
/**
 * Wait for a forked visualizer process to print its URL.
 * Resolves with { url } on success, { error } if the child exits or fails
 * to report a URL within the timeout. Never returns a fabricated URL.
 */
export function waitForVisualizerUrl(child, timeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    const check = () => {
      ...
      if (match) finish({ url: match[1] });
    };
    child.stdout.on('data', (data) => { output += data.toString(); check(); });
    child.stderr.on('data', (data) => { output += data.toString(); check(); });
    child.on('error', (err) => finish({ error: err.message }));
    child.on('exit', (code) => finish({ error: `visualizer exited with code ${code}: ${output.slice(-400).trim()}` }));
    setTimeout(() => finish({ error: `visualizer did not report a URL within ${timeoutMs}ms: ${output.slice(-400).trim()}` }), timeoutMs);
  });
}
