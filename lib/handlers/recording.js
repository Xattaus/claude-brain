  ...
  brain_record_lesson: async (ctx, args) => {
    ...
    const result = await manager.createEntry({
      ...
      frontmatter: {
        ...
        related: relatedInput,
        files: args.files || [],
        // Stored in frontmatter (and copied to index by createEntry) so the
        // rule survives brain_rebuild_index and needs no second locked write.
        ...(args.rule ? { rule: `"${manager.escapeYaml(args.rule)}"` } : {})
      },
      body
    });
