...
export const codeGraphHandlers = {
    ...
    try {
      ...
      if (neighbors.length > 0) {
        ...
        for (const n of neighbors.slice(0, 20)) {
          text += `- ${n.direction === 'outgoing' ? '→' : '←'} ${n.id} [${n.relation || '?'}]\n`;
        }
  ...
  brain_code_visualize: async (ctx) => {
    ...
    if (!projectPath) {
      return [{ type: 'text', text: 'Error launching code graph visualizer: project path is not available.' }];
    }

    try {
      const { waitForVisualizerUrl } = await import('./advanced.js');
      const child = fork(vizScript, [projectPath, '--code'], { silent: true });
      const result = await waitForVisualizerUrl(child);
      if (!result.url) {
        return [{ type: 'text', text: `Error launching code graph visualizer: ${result.error}` }];
      }
      child.unref();
      return [{ type: 'text', text: `## Code Graph Visualizer Launched\n\nOpened interactive code graph in browser.\n\n**URL:** ${result.url}\n\nAlso available: ${result.url}/brain (knowledge graph)\n\nPress Ctrl+C in the terminal to stop the visualizer server.` }];
    } catch (err) {
